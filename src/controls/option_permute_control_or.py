...
The local-inference counterpart is src/controls/option_permute_control.py, used for
qwen-3.5-4b. This script covers the nine OpenRouter-served models.
"""
...
def load_confirm(itype, n, start=0):
    """Return confirmation queries of one claim type, sliced to items[start:start+n]."""
    items = []
...
def permute(query, block):
    """Replace the original option block in a query with a permuted block."""
    return query.replace(ORIG_BLOCK, block)


def add_nofc(query):
    """Insert the no-fact-check instruction before the trailing 'Answer:'."""
    return query.replace("\nAnswer:", f"\n\n{NO_FACT_CHECK}\nAnswer:")


def one(model_or, query):
    """Return the extracted MCQ answer for one query, or None on failure."""
...
def run_cell(model_or, queries, gold, workers):
    """Answer a set of queries concurrently and return (accuracy, n_parsed)."""
...
def run_model(model_or, short, n, workers):
    """Run every permutation, template, and claim type for one model, writing JSON."""
...
def main():
    """Run the OpenRouter answer-position control over the selected models."""
