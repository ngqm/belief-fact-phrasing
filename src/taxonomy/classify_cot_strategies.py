...
def build_judge_prompt():
    """Return the LLM-judge prompt describing the strategy taxonomy and output format."""
    lines = [
...
def classify_one(client, model, item, retries=4):
    """Send one CoT to the judge and return its {category, rationale}, retrying on failure."""
    user = f"{JUDGE_PROMPT}\n\nChain of thought:\n{item['text']}"
...
def main():
    """Classify every CoT in the pool with the judge and write the results JSONL."""
