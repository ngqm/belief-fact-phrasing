...
# Figure palette: terracotta/clay primary, slate, muted sage.
PALETTE = {
...
def fig1_verb_gap():
    """Draw the per-verb factual vs false accuracy figure (fig1) and save it as PDF."""
    ...
    for i, v in enumerate(agg.index):
        ...
        ax.plot(val_f,  i, marker="o", color=PALETTE["slate"],
                markeredgecolor="black", markeredgewidth=0.6, markersize=6, zorder=3)
        ax.plot(val_fa, i, marker="o", color=PALETTE["terra"],
                markeredgecolor="black", markeredgewidth=0.6, markersize=6, zorder=3)
        if val_f >= val_fa:
    ...
    ax.scatter([], [], marker="o", color=PALETTE["slate"],
               edgecolor="black", linewidth=0.6, s=36, label="factual")
    ax.scatter([], [], marker="o", color=PALETTE["terra"],
...
def fig2_confidence():
    """Draw accuracy vs stated confidence for the five largest models (fig2)."""
    df = pd.read_csv("analysis/original/master_accuracy_summary.csv")
    ...
    colors = {
        "gemma-3-12b":       PALETTE["terra_lite"],
        "gemma-3-27b":       PALETTE["terra_dark"],
...
def fig3_strategies():
    """Draw the reasoning-strategy distribution and fact-checking accuracy (fig3)."""
    ...
    axes[0].bar(range(len(strategies)), counts.values, width=0.5, color=PALETTE["grey"])
    ...
    w = 0.22
    axes[1].bar(x - w / 2, acc[:, 0], w, color=PALETTE["slate"], label="factual")
    axes[1].bar(x + w / 2, acc[:, 1], w, color=PALETTE["terra"], label="false",
...
def main():
    """Render all three paper figures to the output directory."""
    fig1_verb_gap()
    fig2_confidence()
    fig3_strategies()
