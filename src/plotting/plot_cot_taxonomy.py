...
def load():
    """Load the classifications and return (DataFrame, strategy list ordered by frequency)."""
...
def plot_overall(df, strategies, out):
    """Plot the overall count of CoTs per strategy and save it to out."""
...
def _share_matrix(df, row_key, row_order, strategies):
    """Return a (row_order x strategies) matrix of each strategy's share within each row group."""
...
def _heatmap(M, row_labels, col_labels, title, out, figsize):
    """Render a heatmap of matrix M with row and column labels and save it to out."""
...
def plot_by_verb(df, strategies, out):
    """Plot a heatmap of strategy share per verb and save it to out."""
...
def plot_by_model(df, strategies, out):
    """Plot a heatmap of strategy share per model and save it to out."""
...
def plot_accuracy_by_type(df, strategies, out):
    """Plot per-strategy accuracy for factual vs false claims and save it to out."""
    types = ["factual", "false"]
...
def plot_factcheck_vs_no_factcheck(df, out):
    """Plot accuracy for fact-checking vs non-fact-checking CoTs, overall and per verb."""
    df = df.copy()
...
def main():
    """Load classifications and write all five taxonomy charts to the output directory."""
