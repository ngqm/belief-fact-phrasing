"""Run the OpenRouter belief evaluation in generate, progress, or evaluate mode."""
import os
import json
import argparse
from openai import OpenAI
from src.utils.general import (get_verb_mappings, get_model_response,
                           load_results, get_all_models)
from src.utils.dataset import Dataset
from src.utils.visualize import plot_performance, plot_performance_am_x_confident
...
def generate(output_base, prompt_template, verb, model, dataset, max_tokens=512):
    """Generate model answers for one verb and model, writing one JSON per item."""
...
def progress(output_base, prompt_template):
    """Print each verb and model whose generated file count is not 1000."""
...
def evaluate(output_base, analysis_base, visualization_base, prompt_template):
    """Load results and render the accuracy and confidence figures."""
