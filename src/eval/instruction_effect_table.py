"""Per-verb-family accuracy of each instruction template.

For each verb in:
  - Positive belief subset: believe, think, suppose, am_certain
  - Evidential family: vaguely_remember
  - Confidence family: am_80_confident
  - Negation family: seriously_doubt
...
Prints both:
  (a) The positive-belief 4-verb-average table.
...
def extract_answer(text):
    """Find the last (A|B|C) in text, matching main.py's parser."""
    matches = ANS_RE.findall(text or "")
    return matches[-1].upper() if matches else None


def parse_gold(gt):
    """Normalize a stored ground_truth ('(A)' or 'A') to a bare letter."""
    m = ANS_RE.search(gt or "")
    return m.group(1).upper() if m else (gt.strip() if gt else None)


def load_cell(template, verb, model):
    """Return a dict mapping claim type to [n_correct, n_parsed, n_total] for one cell."""
...
def cell_accuracy(template, verb, model):
    """Return per-cell accuracy as (acc_factual, acc_false).

    Uses n_total as the denominator, so parse failures count as wrong, matching
    the main accuracy results.
    """
...
def fmt_row(template, fa, fb, n=None):
    """Return a formatted table row line for one template's accuracies."""
    gap = (fa - fb) * 100
...
def main():
    """Print the 4-verb, per-verb, and per-family accuracy tables to stdout."""
    print("=" * 80)
    ...
    print()
    print("=" * 80)
    print("(B) Per-verb breakdown: each verb under all 4 templates")
    print("=" * 80)
