"""Run the KaBLE epistemic-verb evaluation with batched transformers generation.

A plain transformers fallback for machines without a vLLM install.
Batches prompts together inside model.generate() for a large speedup over
...
from src.utils.models import MODELS, load_model, is_qwen3
from src.utils.general import get_verb_mappings, apply_verb


def load_kable(dataset_file):
    """Return the dataset JSONL parsed into a list of dicts."""
    return [json.loads(l) for l in open(dataset_file)]


def existing_indices(out_dir):
    """Return the set of item indices already written under out_dir."""
...
def load_system_prompt(prompt_template):
    """Return the system prompt text for a template, or '' if absent."""
    with open("data/prompt_templates.json") as f:
        return json.load(f).get(prompt_template, "")
...
def run_one_verb(model_name, verb, tok, model, items, prompt_template,
    ...
    """Generate and save answers for every remaining item of one verb, batched."""
    ...
    for bs in tqdm(range(0, len(todo), batch_size),
        ...
        prompts = []
        for _, ex in batch:
            q = apply_verb(ex["query"], verb_map)
            messages = []
            if system_prompt:
...
def main():
    """Parse arguments, load the model, and run batched generation for each verb."""
    ...
    chat_kwargs = {"enable_thinking": False} if is_qwen3(hf_id) else {}
    ...
    print(f"model={args.model}  hf={hf_id}  "
