"""Re-run items whose model output was empty or failed to parse, at a larger token budget.

Overwrites each JSON file in place.
"""
import argparse
import glob
import json
...
def needs_rerun(d):
    """Return True if the record's model_output is empty or unparseable."""
...
def rerun_one(client, model, path, max_tokens):
    """Re-query one item at max_tokens, overwrite its JSON, and return (path, ok, error)."""
...
def main():
    """Find empty or unparseable items across templates and re-run them concurrently."""
