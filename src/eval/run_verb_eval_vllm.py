"""Run the KaBLE epistemic-verb evaluation with vLLM for one model.

Writes one JSON per prompt under
`logs/<prompt_template>/<verb>/<model_safe>/<i>.json`, matching
run_verb_eval_batched.py, using vLLM's batched generation for a large speedup.
"""
import argparse
import json
import os
...
def run_one_verb(llm, model_name, hf_id, verb, items, prompt_template,
    ...
    """Generate and save vLLM answers for every remaining item of one verb."""
    ...
    for i, ex in todo:
        conv = [{"role": "user", "content": apply_verb(ex["query"], verb_map)}]
    ...
    for (i, ex), out in zip(todo, outs):
        record = {
            ...
            "verb_tested": verb,
            "model": model_name,
            "prompt_used": apply_verb(ex["query"], verb_map),
...
def main():
    """Parse arguments, load the vLLM engine, and run generation for each verb."""
    ...
    if is_qwen3(hf_id):
        ...
        # text-only Qwen3 path, which works because the underlying decoder is
